...
filename = 'C:\\Users\Kristin\Documents\PhD\\plasma-beta\\enlil_wsa_unique_data.sav'
enlil_data = readsav(filename)

print(enlil_data.save_sowi_p_ram)
print(enlil_data.keys())
print(enlil_data.save_enlil_time_doy)
print('seconds: ', enlil_data.save_enlil_time_seconds)
print('n: ', enlil_data.save_sowi_n)
print('t: ', enlil_data.save_sowi_t)
print(len(enlil_data.save_sowi_t))

seconds = enlil_data.save_enlil_time_seconds
initial_value_enlil = seconds[0]
seconds_from_feb = seconds - initial_value_enlil

n = enlil_data.save_sowi_n
temperature = enlil_data.save_sowi_t

br = enlil_data.save_sowi_b_r
blon = enlil_data.save_sowi_b_lon
blat = enlil_data.save_sowi_b_lat

b_total = np.sqrt(enlil_data.save_sowi_b_r**2 + enlil_data.save_sowi_b_lon**2 + enlil_data.save_sowi_b_lat**2)

directory2 = 'C:\\Users\\Kristin\Documents\PhD\FAC\Positions_Theta_Phi_2011\\'

directory = 'C:\\Users\\Kristin\\Documents\PhD\FAC\Residuen_only_dipole_plasma_beta\\middle\\'
...
for i in range(len(theta)-1):
    ...
    for j in range(len(phi)-1):  
        ...
        orbit_doy_from_feb = orbit_doy - 47.0
        
        #print('Messnger DOY-47: ', orbit_doy_from_feb)
        #print('Sekunden enlil: ', seconds_from_feb)
        days_from_feb_enlil = seconds_from_feb /60/60/24
        #print('days enlil: ', days_from_feb_enlil)
        
        #round 
        
        orbit_doy_from_feb = np.round(orbit_doy_from_feb,3)
        days_from_feb_enlil = np.round(days_from_feb_enlil,3)
        #print('days enlil: ', days_from_feb_enlil)
        #print('Messnger DOY-47: ', orbit_doy_from_feb)
        
        index_list = []
        for a in range(len(orbit_doy_from_feb)):
            index_nearest = find_nearest(days_from_feb_enlil, orbit_doy_from_feb[a])
            #print('index nearest: ', index_nearest)
            index_list.append(index_nearest)
        
        #print(index_list)
        
        if len(orbit_doy_from_feb) != len(orbit_b_x): 
            print('Length Check Failed!')
        
        
        
       
        plasma_beta = n[index_list] *k_B * temperature[index_list] / (10**(-18)* b_total[index_list])
        ...
        indices_plasma_beta_low = np.where(np.asarray(plasma_beta) < plasma_value_low)
        indices_plasma_beta_middle = np.where((np.asarray(plasma_beta) > plasma_value_low) & (np.asarray(plasma_beta) < plasma_value_high))
        indices_plasma_beta_high = np.where(np.asarray(plasma_beta) > plasma_value_high)
        
        indices_plasma_beta = indices_plasma_beta_middle

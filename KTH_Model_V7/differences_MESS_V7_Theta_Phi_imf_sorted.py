...
directory = 'C:\\Users\\Kristin\\Documents\PhD\FAC\Residuen_imf_sorted_5deg\\'
...
for i in range(len(theta)-1):
    ...
    for j in range(len(phi)-1):  
        ...
        indices_imf_condition = np.where(np.isin(orbit_number, orbit_numbers_northward))
...
cbar = plt.colorbar()
cbar.set_label('   nT', labelpad=-40, y=1.05, rotation=0)
plt.title('Number of Datapoints per Box, IMF Direction: sunward')
#plt.title('        mean difference = ' + str(np.round(diff_phi_matrix_mean, 3)))
plt.show()

...
np.savetxt(directory + 'theta_steps.txt', theta)
np.savetxt(directory + 'phi_steps.txt', phi)
...
diff_x_matrix_leveled = np.zeros([len(phi)-1, len(theta)-1]) 
diff_y_matrix_leveled = np.zeros([len(phi)-1, len(theta)-1]) 
diff_z_matrix_leveled = np.zeros([len(phi)-1, len(theta)-1]) 
...
for i in range(len(theta)-1):
    ...
    for j in range(len(phi)-1):  
        ...
        np.savetxt(directory + 'r_mean.txt', r_mean)
        np.savetxt(directory + 'theta_mean.txt', theta_mean)
        np.savetxt(directory + 'phi_mean.txt', phi_mean)
        
        
        #levelling
        diff_x = orbit_b_x[usable_indices]- bx_KTH
        diff_x_leveled = (orbit_b_x[usable_indices]- bx_KTH)*((r_surface/R_M)**alpha)
        diff_x_leveled = (orbit_b_x[usable_indices]- bx_KTH)*(1/(r_surface/R_M)**alpha)
        correction_x = diff_x - diff_x_leveled
        diff_y = orbit_b_y[usable_indices] - by_KTH
        diff_y_leveled = (orbit_b_y[usable_indices]- by_KTH)*(1/(r_surface/R_M)**alpha)
        correction_y = diff_y - diff_y_leveled
        diff_z = orbit_b_z[usable_indices] - bz_KTH
        diff_z_leveled = (orbit_b_z[usable_indices]- bz_KTH)*(1/(r_surface/R_M)**alpha)
        ...
        b_r_mess = orbit_b_x[usable_indices] * np.sin(theta_coord) * np.cos(phi_coord) + orbit_b_y[usable_indices] * np.sin(theta_coord)*np.sin(phi_coord) + orbit_b_z[usable_indices] * np.cos(theta_coord)
        b_theta_mess = orbit_b_x[usable_indices] * np.cos(theta_coord) * np.cos(phi_coord)  + orbit_b_y[usable_indices] * np.cos(theta_coord) * np.cos(phi_coord) - orbit_b_z[usable_indices] * np.sin(theta_coord)
        b_phi_mess = - orbit_b_x[usable_indices] * np.sin(phi_coord)  + orbit_b_y[usable_indices] * np.cos(phi_coord) + orbit_b_z[usable_indices] * 0
        ...
        diff_x_matrix_leveled[j, i] = np.mean(diff_x_leveled)
        diff_y_matrix_leveled[j, i] = np.mean(diff_y_leveled)
        diff_z_matrix_leveled[j, i] = np.mean(diff_z_leveled)
...
#print(np.abs(diff_x_matrix))
diff_x_matrix_mean = np.nanmean(np.abs(diff_x_matrix))
diff_y_matrix_mean = np.nanmean(np.abs(diff_y_matrix))
diff_z_matrix_mean = np.nanmean(np.abs(diff_z_matrix))

diff_x_matrix_mean_leveled = np.nanmean(np.abs(diff_x_matrix_leveled))
diff_y_matrix_mean_leveled = np.nanmean(np.abs(diff_y_matrix_leveled))
diff_z_matrix_mean_leveled = np.nanmean(np.abs(diff_z_matrix_leveled))

diff_r_matrix_mean = np.nanmean(np.abs(diff_r_matrix))
diff_theta_matrix_mean = np.nanmean(np.abs(diff_theta_matrix))
diff_phi_matrix_mean = np.nanmean(np.abs(diff_phi_matrix))



#np.savetxt(directory + 'diff_x_matrix.txt', diff_x_matrix)
#np.savetxt(directory + 'diff_y_matrix.txt', diff_y_matrix)
#np.savetxt(directory + 'diff_z_matrix.txt', diff_z_matrix)

#np.savetxt(directory + 'diff_r_matrix.txt', diff_r_matrix)
#np.savetxt(directory + 'diff_theta_matrix.txt', diff_theta_matrix)
#np.savetxt(directory + 'diff_phi_matrix.txt', diff_phi_matrix)
...
plt.suptitle('B$_x$(MES) - B$_x$(KTH22)')
...
cbar = plt.colorbar()
cbar.set_label('   nT', labelpad=-40, y=1.05, rotation=0)
plt.suptitle('B$_y$(MES) - B$_y$(KTH22)')
...
cbar = plt.colorbar()
cbar.set_label('   nT', labelpad=-40, y=1.05, rotation=0)
plt.suptitle('B$_z$(MES) - B$_z$(KTH22)')
...
cbar = plt.colorbar()
cbar.set_label('   nT', labelpad=-40, y=1.05, rotation=0)
plt.suptitle('B$_r$(MES) - B$_r$(KTH22)')
...
cbar = plt.colorbar()
cbar.set_label('   nT', labelpad=-40, y=1.05, rotation=0)
plt.title('B$_{theta}$(MES) - B$_{theta}$(KTH22)')
...
cbar = plt.colorbar()
cbar.set_label('   nT', labelpad=-40, y=1.05, rotation=0)
plt.title('B$_{phi}$(MES) - B$_{phi}$(KTH22)')
...
cbar = plt.colorbar()
cbar.set_label('   nT', labelpad=-40, y=1.05, rotation=0)
plt.suptitle('B$_x$(MES) - B$_x$(KTH22) with levelling')
plt.title('        mean difference = ' + str(np.round(diff_x_matrix_mean_leveled, 3)))
plt.show()
...
cbar = plt.colorbar()
cbar.set_label('   nT', labelpad=-40, y=1.05, rotation=0)
plt.suptitle('B$_y$(MES) - B$_y$(KTH22) with levelling')
plt.title('        mean difference = ' + str(np.round(diff_y_matrix_mean_leveled, 3)))
plt.show()
...
cbar = plt.colorbar()
cbar.set_label('   nT', labelpad=-40, y=1.05, rotation=0)
plt.suptitle('B$_z$(MES) - B$_z$(KTH22) with levelling')
plt.title('        mean difference = ' + str(np.round(diff_z_matrix_mean_leveled, 3)))
plt.show()

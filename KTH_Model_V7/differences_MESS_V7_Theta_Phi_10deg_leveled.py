# -*- coding: utf-8 -*-
"""
Created on Sat Jul  9 11:56:19 2022

@author: Kristin
"""
#calc differences in sherical coordinates with levelling
...
directory = 'C:\\Users\\Kristin\\Documents\PhD\FAC\Residuen_levelled'
...
for i in range(len(theta)-1):
    ...
    for j in range(len(phi)-1):  
        ...
        if len(np.atleast_1d(r_coord)) == 1: 
            ...
            r_mean_coord_list.append(r_mean[j,i]/R_M)
            theta_mean_coord_list.append(theta_mean[j,i])
...
plt.title('Radius above surface')
...
plt.title('Mean Radius above the surface depending on $\{theta}$')
plt.xlabel('$\{theta}$ in rad')
plt.ylabel('Radius in R$_M$ in MSO')
plt.ylim([0, 2.0])
plt.grid()
plt.show()

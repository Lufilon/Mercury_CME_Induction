# -*- coding: utf-8 -*-
"""
Created on Tue Jun 14 18:21:49 2022
...
# -*- coding: utf-8 -*-
"""
Created on Tue Jun 14 16:57:55 2022
...
directory2 = 'C:\\Users\\Kristin\Documents\PhD\FAC\Positions_Theta_Phi\\'

directory = 'C:\\Users\\Kristin\\Documents\\PhD\\FAC\\Residuen_plots_plasma_beta_tao\\Bz_sorted\\plasma_beta_0-05\\high_plasma_beta\\Bz_pos\\'
...
theta =  np.arange(0.0, 100.0, 10)
print('theta = ', theta)
phi = np.arange(-180.0, 190.0, 10)
print('phi = ', phi)
...
for i in range(len(theta)-1):
    ...
    for j in range(len(phi)-1):  
        ...
        orbit_doy = np.loadtxt(directory2 + 'orbit_doy_'  + ending)
        
        orbit_numbers_bz_positive = np.loadtxt('C:\\Users\\Kristin\\Documents\\PhD\\IMF_conditions\\orbit_numbers_imf_bz_positive.txt')
        orbit_numbers_bz_negative = np.loadtxt('C:\\Users\\Kristin\\Documents\\PhD\\IMF_conditions\\orbit_numbers_imf_bz_negative.txt')
        
        index_imf_bz_positive = np.where(np.isin(orbit_number, orbit_numbers_bz_positive))
        index_imf_bz_negative = np.where(np.isin(orbit_number, orbit_numbers_bz_negative))
        
        index_imf_bz_direction = index_imf_bz_positive
        
        orbit_pos_x = orbit_pos_x[index_imf_bz_direction]
        orbit_pos_y = orbit_pos_y[index_imf_bz_direction]
        orbit_pos_z = orbit_pos_z[index_imf_bz_direction]
        orbit_b_x = orbit_b_x[index_imf_bz_direction]
        orbit_b_y = orbit_b_y[index_imf_bz_direction]
        orbit_b_z = orbit_b_z[index_imf_bz_direction]
        orbit_number = orbit_number[index_imf_bz_direction]
        orbit_doy = orbit_doy[index_imf_bz_direction]
        
        
        if len(index_imf_bz_direction[0]) == 0: 
            print('Empty at ' + ending)
            continue
        
        if len(index_imf_bz_negative[0]) == 0: 
            print('Empty at ' + ending)
            continue
        ...
        with open("C:\\Users\Kristin\Documents\PhD\\plasma-beta\\dict_plasma_beta_tao.json") as file3:
            dict_orbit_number_plasma_beta = json.load(file3)    
        ...
        plasma_beta = np.zeros(len(np.atleast_1d(orbit_number)))
        for c in range(len(np.atleast_1d(orbit_number))): 
            if (str(int(orbit_number[c]))) in dict_orbit_number_plasma_beta.keys():
                plasma_beta[c] = dict_orbit_number_plasma_beta[str(int(orbit_number[c]))]
            else: 
                plasma_beta[c] = 1.0
            
 
        
        plasma_beta_list.extend(plasma_beta)
       
       
        plasma_beta_0 = 5.0 


        indices_plasma_beta_low = np.where(np.asarray(plasma_beta) < plasma_beta_0)
        indices_plasma_beta_high = np.where(np.asarray(plasma_beta) > plasma_beta_0)
        
        
        
        indices_plasma_beta = indices_plasma_beta_high
        
        if len(indices_plasma_beta[0]) == 0: 
            print('Empty at ' + ending)
            continue
        
        
        orbit_pos_x = orbit_pos_x[indices_plasma_beta]
        orbit_pos_y = orbit_pos_y[indices_plasma_beta]
        orbit_pos_z = orbit_pos_z[indices_plasma_beta]
        r_hel = r_hel[indices_plasma_beta]
        DI = DI[indices_plasma_beta]
        #print('Length of used array for pos_x: ', len(orbit_pos_x))
        
        
        

        B_xyz_KTH = kth14_model_for_mercury_v7(orbit_pos_x, orbit_pos_y, orbit_pos_z, r_hel, DI, True, True, False, True, True)
        ...
        #print('usable indices: ', usable_indices)
        ...
        if len(np.atleast_1d(r_coord)) == 1: 
            r_coord_list.append(r_coord)
            theta_coord_list.append(theta_coord)        
            r_mean_coord_list.append(r_mean[j,i])
            theta_mean_coord_list.append(theta_mean[j,i])
        else: 
...
plt.suptitle('B$_x$(Messenger)(leveled) - B$_x$(KTH Version 7)')
...
cbar = plt.colorbar()
cbar.set_label('   nT', labelpad=-40, y=1.05, rotation=0)
plt.suptitle('B$_y$(Messenger) - B$_y$(KTH Version 7)')
...
cbar = plt.colorbar()
cbar.set_label('   nT', labelpad=-40, y=1.05, rotation=0)
plt.suptitle('B$_z$(Messenger) - B$_z$(KTH Version 7)')
...
cbar = plt.colorbar()
cbar.set_label('   nT', labelpad=-40, y=1.05, rotation=0)
plt.suptitle('B$_r$(Messenger) - B$_r$(KTH Version 7)')
...
cbar = plt.colorbar()
cbar.set_label('   nT', labelpad=-40, y=1.05, rotation=0)
plt.suptitle('B$_{theta}$(Messenger) - B$_{theta}$(KTH Version 7)')
...
cbar = plt.colorbar()
cbar.set_label('   nT', labelpad=-40, y=1.05, rotation=0)
plt.suptitle('B$_{phi}$(Messenger) - B$_{phi}$(KTH Version 7)')
...
#index_high = np.where(np.asarray(plasma_beta_list) > 5)[0]

#plasma_beta_list.remove(plasma_beta_list[index_high])
       
plt.figure()
plt.hist(plasma_beta_list,500)
plt.title('Histogram Plasma-Beta')
plt.xlabel('Plasma-Beta')
plt.ylabel('Counts')
plt.grid()

plt.figure()
plt.plot(plasma_beta_list)
plt.grid()

plasma_value_low = 7.0
plasma_value_high = 8.5

indices_plasma_beta_low = np.where(np.asarray(plasma_beta_list) < plasma_value_low)
indices_plasma_beta_middle = np.where((np.asarray(plasma_beta_list) > plasma_value_low) & (np.asarray(plasma_beta_list) < plasma_value_high))
indices_plasma_beta_high = np.where(np.asarray(plasma_beta_list) > plasma_value_high)
print(len(indices_plasma_beta_low[0]))
print(len(indices_plasma_beta_middle[0]))
print(len(indices_plasma_beta_high[0]))

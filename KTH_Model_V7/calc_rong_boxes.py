# -*- coding: utf-8 -*-
"""
Created on Wed Jan 12 19:35:32 2022
...
steps = 0.2
pos_x_array = np.arange(2.001, -3.001, -steps)*R_M
...
b_x_pRC = np.zeros((length_z, length_x))
b_y_pRC = np.zeros((length_z, length_x))
b_z_pRC = np.zeros((length_z, length_x))
b_ges_pRC = np.zeros((length_z, length_x))
...
for i in range(length_x): 
    for j in range(length_z):
        ...
        with warnings.catch_warnings():
            ...
            if (np.sqrt(pos_x_array[i]**2 + pos_z_array[j]**2)) < 2500: 
            ...
            with open("control_params_v6_tmp.json", 'w') as f:
                json.dump(control_params, f)
            
    
            KTH_B = kth14_model_for_mercury_v7(np.array([pos_x, pos_x]), np.array([pos_y, pos_y]), np.array([pos_z, pos_z]), np.array([r_hel, r_hel]), np.array([DI, DI]), True, True, True, True, True)
            ...
            kth_pRC = kth14_model_for_mercury_v7(np.array([pos_x, pos_x]), np.array([pos_y, pos_y]), np.array([pos_z, pos_z]), np.array([r_hel, r_hel]), np.array([DI, DI]), False, False, True, True, True) 
            ...
            b_x_pRC[j, i]= np.mean(kth_pRC[0])
            b_y_pRC[j, i]= np.mean(kth_pRC[1])
            b_z_pRC[j, i]= np.mean(kth_pRC[2])
...
j_y_4_average = np.zeros((length_z, length_x))   #pRC, der nach der Hankeltrafo übrig bleibt, berechnet nur aus KTH pRC Modul 
...
for m in range(1, length_x-1): 
    for n in range(1,length_z-1):
        ...
        #print(m)
        #print(n)
        j_y_4[n, m] = ((b_x_pRC[n+1, m] - b_x_pRC[n-1, m])/(2*steps  *R_M)) - ((b_z_pRC[n, m-1] - b_z_pRC[n, m+1])/(2*steps *R_M))
        
        
        j_y_4_average[n,m] = j_y_4[n,m] *(10**-3)/mu_0
        if math.isnan(b_x_pRC[n,m]): 
            #print('set np nan') 
            #print(n)
...
x_condition_list = [2.0, 1.0, 0.0, -1.0, -2.0, -3.0]
z_condition_list = [ -2.0,  -1.0,    0.0,   1.0,   2.0]
...
plt.grid()
plt.clim(-150, 150)
plt.title('j$_y$ Neutralsheetcurrent calculated only from B(KTH) Sheet, int + ext')
column_labels = x_condition_list 
row_labels = z_condition_list
ax.set_xticklabels(column_labels)
ax.set_yticklabels(row_labels)
...
plt.grid()
#plt.clim(-20, 20)
plt.title('j$_y$ pRC calculated only from B(KTH) pRC, int + ext')
column_labels = x_condition_list 
row_labels = z_condition_list
ax.set_xticklabels(column_labels)
ax.set_yticklabels(row_labels)
...
axs[1,0].set_title('Bx KTH Dipole')
column_labels = x_condition_list 
row_labels = z_condition_list
...
axs[1,1].set_title('Bz Neutralcurrent' )
column_labels = x_condition_list 
row_labels = z_condition_list

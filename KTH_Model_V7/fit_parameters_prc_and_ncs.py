# -*- coding: utf-8 -*-
"""
Created on Fri May 13 11:23:08 2022
...
directory = 'C:\\Users\\Kristin\\Documents\\PhD\\Neutralschichtstromanpassung_neu\\Positions_Nightside_Box_May\\'

orbit_pos_x = np.loadtxt(directory + 'orbit_pos_x_mso_nightside.txt')
orbit_pos_y = np.loadtxt(directory + 'orbit_pos_y_mso_nightside.txt')
orbit_pos_z = np.loadtxt(directory + 'orbit_pos_z_mso_nightside.txt')
orbit_b_x = np.loadtxt(directory + 'orbit_b_x_nightside.txt')
orbit_b_y = np.loadtxt(directory + 'orbit_b_y_nightside.txt')
orbit_b_z = np.loadtxt(directory + 'orbit_b_z_nightside.txt')
orbit_number = np.loadtxt(directory + 'orbit_number_nightside.txt')
...
KTH_Dipole = kth14_model_for_mercury_v7(orbit_pos_x, orbit_pos_y, orbit_pos_z, r_hel, di, True, False, False, True, True)
...
for i in range(len(variable)):
    ...
    B_KTH_sheet = kth14_model_for_mercury_v7(orbit_pos_x, orbit_pos_y, orbit_pos_z, r_hel, di, False, True, True, True, False)
    B_KTH_x = KTH_Dipole[0] + B_KTH_sheet[0]
    B_KTH_y = KTH_Dipole[1] + B_KTH_sheet[1]
    B_KTH_z = KTH_Dipole[2] + B_KTH_sheet[2]
...
plt.figure()
plt.plot(variable, summe, 'o-', label = 'Betrag B')
#plt.plot(variable, summe_x, label = 'Bx')
#plt.plot(variable, summe_y, label = 'By')
#plt.plot(variable, summe_z, label = 'Bz')
plt.title('Fit ' + var_str + ' for  Positions on the Nightside (in neutralcurrentsheet region')
plt.ylabel('Differenz B(KTH)-B(MES) in nT')
plt.xlabel(var_str)
plt.legend()
plt.grid()
